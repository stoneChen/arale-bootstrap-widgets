define("value/upload/1.0.0/upload-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug");
    var doc = document, $body = $("body"), getUID = function() {
        var id = 0;
        return function() {
            return "Upload" + id++;
        };
    ...
    Upload.prototype = {
        ...
        createFileEl: function() {
            ...
            var $file = $('<input type="file" class="hide" id="' + self.btn.prop("for") + '" name="' + self.fieldName + '" accept="' + accepts.join(",") + '"/>'), fileChange = function() {
                var $file = $(this);
                if (!self.checkFileType($file.val())) {
                    //检查文件类型
                    $file.remove();
                    //
                    self.createFileEl();
                ...
                if (self.onSubmit) {
                    if (!self.onSubmit(self.btn, $file)) {
                        //返回false,则不提交表单
                        return;
                    }
                }
        ...
        createForm: function() {
            var self = this;
            var uID = getUID();
            var $iframe = $('<iframe frameborder="0"  style="display:none;" name="hiddenIframe' + uID + '" class="hide"></script></iframe>'), $form = $('<form action="' + this.formAction + '" target="hiddenIframe' + uID + '" enctype="multipart/form-data" method="post" style="display:none;"></form>');
            $iframe.on("load", function() {
                ...
                //iDoc.domain = doc.domain;//todo 跨域问题
        ...
        checkFileType: function(filepath) {
            var allow = this.fileType.allow, forbidden = this.fileType.forbidden, ext = filepath.match(/^.*\.(.+)$/);
            if (!ext) {
                return false;
            }
            if (allow) {
                //默认allow,也可设置禁止项,但两者不能共存
        ...
        createOtherData: function() {
            var otherParams = this.otherParams, htmlText = "";
